...
{$IFDEF UseNativeDraw} FMX.Graphics.Native, {$ENDIF}
  ...
  FMX.Presentation.Messages, FMX.Styles.Objects, FMX.Presentation.Style, FMX.Controls.Presentation, FMX.Controls.Model, MD.Input,
...
begin
  ...
  try
    ...
    begin
      if Supports(FPromptText, ICaption, Caption) then
        Caption.Text := Model.TextPrompt;
      FPromptText.SetBounds(16, 0, Self.Width, Self.Height);
      FPromptText.Font.Size := 20;
      FPromptText.FontColor := $FF757575;
      FPromptText.Opacity := 1;
      FPromptText.HitTest := False;
      FPromptText.VertTextAlign := TTextAlign.Center;
      // FPromptText. HorzTextAlign := TTextAlign.Leading;
      ...
      FPromptFloatAnimation.StartValue := 0;
      FPromptFloatAnimation.StopValue := Self.Height / 2 * -1;
...
begin
  ...
  FOutlinedPathData.CurveTo(TPointF.Create(Self.Width - 2, Self.Height - 2), TPointF.Create(Self.Width - 2, Self.Height - 2),
    TPointF.Create(Self.Width - 14, Self.Height - 2));
  ...
  FOutlinedPathData.Translate(FContent.Margins.Left * -1 + 2, FContent.Margins.Top * -1 + 2);
end;
...
begin
  ...
  begin
    Canvas.Stroke.Color := $FFBA2922;
    FPromptText.FontColor := $FFBA2922;
      ...
      begin
        ...
        for i := Low(FSpellingRegions) to High(FSpellingRegions) do
          Canvas.DrawLine(TPointF.Create(FSpellingRegions[i].Left - VisibleCharPos, FSpellingRegions[i].Bottom),
            TPointF.Create(FSpellingRegions[i].Right - VisibleCharPos, FSpellingRegions[i].Bottom), AbsoluteOpacity, FSpellUnderlineBrush);
      end;
  ...
  try
    ...
    begin
      ...
      begin
        if (ParentControl.IsFocused) then
        begin
          FPromptText.SetBounds(16, FPromptFloatAnimation.StopValue, Self.Width, Self.Height);
          FPromptText.TextSettings.Font.Size := 16;
        end;
        FPromptText.Visible := True;
      end;
...
begin
  ...
  begin
    if FPromptText <> nil then
    begin
      FPromptText.SetBounds(16, Self.Height / 2 * -1, Self.Width, Self.Height);
      FPromptText.Visible := True;
      FPromptText.TextSettings.Font.Size := 16;
    end;
    FNeedBuildOutlinedPathData := True;
    ...
    case ASelectionPointType of
      TSelectionPointType.Left:
        begin
          ...
          LoupePos := SelectionRect.TopLeft + TPointF.Create(-FLoupeService.GetWidth / 2, -FLoupeService.GetHeight) +
    ...
    begin
      ...
      begin
        ...
        if MarkedPosition < (FFirstVisibleChar - 1) then
          ...
          while (TextWidth(FFirstVisibleChar - 1, MarkedPosition - FFirstVisibleChar + 1) > LEditRect.Width) and
